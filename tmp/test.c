...
{
    ElemType *p = realloc(S->stack, 2*S->MaxSize*sizeof(ElemType));//?????????????2?
    if (!p)  //??????
    {
        printf("??????!\n");
        exit(1);
...
#elif defined d_23	//binary search
//	it can only be used for sorted array! If you want to use it on unsorted array, you have to sort the array first.
int main()
{
