...
int main(void)
{
	...
	pHead = Create_LinkedList(data[0]);
	showme(pHead);

	Insert_Node_Front(4, 300);
	showme(pHead);

	Insert_Node_Behind(5, 500);
	showme(pHead);
}

#elif defined yy
int longest_unrepeat_substring(char *str)
{
	int len = strlen(str);
	...
	for(int i = 0; i < len; i++)
	{
		substr[i] = (char *)malloc(sizeof(char) * len);
		memset(substr[i], 0, sizeof(char) * len);
	}
...
int main(void)
{
    ...
    int len = longest_unrepeat_substring(str);
    printf("%d\n", len);
