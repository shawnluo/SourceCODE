...
/*
    find the tail, then tail->next point to nth ll
*/
pData create_circular(pData pHead, int n)
{
    pData p = pHead;
    pData tmp = pHead;
    while(p->next != NULL)
...
/*
    1. use 2 pointer, p1 speed is twice of p2. when p1 meet p2, then keep p2, p1 points to the head
    2. then p1 and p2 travel the linked list with the same speed, when they meet, then the meet point is the entry
*/
int fnd_circular_entry(pData pHead)
{
    assert(pHead);
    pData p1, p2;
    ...
    for(p1 = pHead->next, p2 = pHead->next->next; p2 != NULL; p1 = p1->next, p2 = p2->next->next)
    {
        if(p1 == p2)
        {
            break;
    ...
    for(p1 = pHead; ; p1 = p1->next, p2 = p2->next)
    {
        if(p1 == p2)
        {
            break;
...
//    fnd_element(pHead, 2);
    ...
    create_circular(pHead1, 1);
    //showme(pHead1);

    //int i = pHead1->next->next->id;
    //printf("%d\n", i);

    int val = fnd_circular_entry(pHead1);
    printf("%d\n", val);
